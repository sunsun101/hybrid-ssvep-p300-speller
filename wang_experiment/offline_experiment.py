...
def flicker(board):
    ...
    for target in sequence:
        ...
        print("Here is the target flicker size", target_flicker.size)
        ...
        if count == 3 :
            for t in targets.values():
                t.autoDraw = False
            drawTextOnScreen("Break for 1 min", window)
            core.wait(60)
            count = 0
...
def main():
    ...
    while True:
        ...
        a.hear('A_')
        drawTextOnScreen("Starting Experiment.Please donot move now",window)
        ...
        save_raw(raw,PARTICIPANT_ID,RECORDING_DIR, PARTICIPANT_ID)
